# demo.opsradar.io — VPS setup

This directory holds the bits needed to run the OpsRadar public demo
on a dedicated VPS. The demo is a full OpsRadar instance with:

- A pre-seeded **demo user** (`demo@demo.opsradar.io`, locked,
...
# Clone the repo
sudo mkdir -p /srv/opsradar && sudo chown "$USER":"$USER" /srv/opsradar
git clone https://github.com/Absolute-Martial/Ops-Radar /srv/opsradar
cd /srv/opsradar
```
...
OPS_RADAR_ENCRYPTION_KEY=$(openssl rand -hex 32)

# ─ Public-facing host ────────────────────────────────────────
CORS_ORIGINS=https://demo.opsradar.io

# ─ LLM for Ask AI (demo visitors don't bring their own key) ─
OPS_RADAR_LLM_PROVIDER=openrouter
...
Point `demo.opsradar.io` at the VPS public IP (A record for IPv4,
...
sudo cp /srv/opsradar/deploy/demo/Caddyfile /etc/caddy/Caddyfile
sudo systemctl reload caddy
```

That's it. Caddy handles cert issuance + renewal automatically, and
reverse-proxies `https://demo.opsradar.io` → the SPA container on
`127.0.0.1:3000`.
...
register the `opsradar-demo.service` unit so a full OS boot brings
the stack up even if the Docker daemon restart policy breaks:

```bash
sudo cp /srv/opsradar/deploy/demo/opsradar-demo.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now opsradar-demo.service
```
...
curl -s https://demo.opsradar.io/api/v1/demo/status
```
...
```bash
sudo -e /srv/opsradar/.env  # set new OPENROUTER_API_KEY
sudo systemctl restart opsradar-demo.service
```
...
| `opsradar-demo.service` | Systemd unit that runs `docker compose up` on boot. |
