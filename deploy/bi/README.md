# OpsRadar BI connectors

Live data connectors for **Power BI** and **Tableau Desktop**. Both pull
directly from the OpsRadar REST API (`/api/v1/bi/*`) using a standard
...
2. **Home** → **Advanced Editor** → paste the full contents of
   [`opsradar.pq`](./opsradar.pq).
3. **Done** → in the formula bar, call:
   ```
   = OpsRadar.Contents("https://your-opsradar.example.com", "fmk_YOUR_API_KEY")
   ```
4. Expand the returned record — you'll see an `EventLogs` table plus a
   ...
   the event log ID:
   ```
   = OpsRadar.Contents(...)[GetVariants]("00000000-0000-0000-0000-000000000000")
   ```

Generate an API key at **Settings → API Keys** inside OpsRadar. Keys
are scoped to your user's projects — this connector cannot see anything
you cannot see in the web UI.

### Refresh semantics
Mining results are cached in OpsRadar for 15 minutes per event log.
...
1. Host [`opsradar-wdc.html`](./opsradar-wdc.html) on any HTTPS endpoint
...
- The `/api/v1/bi/*` routes reuse the same row-level authorization as
  the rest of OpsRadar's REST API — you cannot read a log you cannot
  access in the UI.
