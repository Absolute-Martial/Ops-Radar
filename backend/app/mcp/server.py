...
user by setting ``OPS_RADAR_MCP_USER_EMAIL`` in the environment — the
...
_TOOL_SCHEMAS: list[Tool] = [
    ...
    Tool(
        ...
        description=(
            ...
            "OpsRadar chat endpoint. Returns a fully narrated answer "
...
async def _resolve_user() -> None:
    """Load the OpsRadar user named by OPS_RADAR_MCP_USER_EMAIL."""
    ...
    email = (
        os.getenv("OPS_RADAR_MCP_USER_EMAIL")
        or os.getenv("FLOWMINER_MCP_USER_EMAIL")
        or ""
    ).strip()
    if not email:
        raise RuntimeError(
            "OPS_RADAR_MCP_USER_EMAIL is required. Set it to the email "
            "of the OpsRadar user whose logs the MCP server should "
            "expose."
        )
...
async def _tool_ask_natural_language(args: dict) -> list[TextContent]:
    """Delegate to the OpsRadar chat endpoint by running the same
...
def _build_server() -> Server:
    server: Server = Server("opsradar-mcp")
...
def main() -> None:
    try:
        ...
        logger.exception("opsradar-mcp crashed: %s", e)
        sys.exit(1)
