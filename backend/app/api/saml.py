...
Endpoints:
  ...
  POST /auth/saml/acs         — Assertion Consumer Service; exchange
                                the SAMLResponse for an OpsRadar JWT
"""
...
def _require_saml():
    if not _saml_enabled():
        raise HTTPException(
            ...
            detail="SAML SSO is not configured on this OpsRadar instance",
        )
...
def _saml_settings() -> dict:
    ...
    return {
        ...
        "debug": False,
        "sp": {
            "entityId": os.getenv("SAML_SP_ENTITY_ID", "opsradar"),
...
async def saml_acs(
    ...
    create the matching user, and return an OpsRadar JWT."""
    _require_saml()
    try:
        from onelogin.saml2.auth import OneLogin_Saml2_Auth
    except ImportError:
