...
Both features are gated behind configuration — OpsRadar ships with
...
SSO flow (authorization code with PKCE):
    ...
    GET /auth/sso/callback       → exchange code, issue OpsRadar JWT
...
def _require_sso_configured():
    if not SSO_ENABLED:
        raise HTTPException(
            ...
            detail="SSO is not configured on this OpsRadar instance",
        )
...
async def sso_callback(body: SSOCallback, db: AsyncSession = Depends(get_db)):
    ...
    issue a OpsRadar JWT. If the email doesn't exist, a new analyst-role
...
def _provisioning_uri(email: str, secret: str, issuer: str = "OpsRadar") -> str:
