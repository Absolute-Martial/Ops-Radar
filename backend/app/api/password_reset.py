...
async def forgot_password(
    ...
    body_text = (
        f"Someone requested a password reset for your OpsRadar account.\n\n"
        f"If this was you, use this token within {RESET_TTL_MINUTES} minutes:\n\n"
        f"{token}\n\n"
        f"If you didn't request this, ignore this email."
    )
    sent = _send_email(user.email, "OpsRadar password reset", body_text)
...
async def send_verification(
    ...
    sent = _send_email(
        user.email,
        "Verify your OpsRadar email",
        f"Welcome to OpsRadar!\n\nVerify your email by submitting this token:\n\n{token}\n",
    )
    if not sent:
        if settings.ENV.lower() == "production":
            logger.warning(
