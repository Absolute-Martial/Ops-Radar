...
def send_scheduled_report(report_id: str):
    ...
    try:
        ...
        msg = MIMEMultipart()
        msg["Subject"] = f"[OpsRadar] Scheduled Report: {report.name}"
        msg["From"] = getattr(settings, "SMTP_FROM", "reports@opsradar.local")
...
def stream_audit_to_siem():
    ...
    try:
        ...
        try:
            ...
            last_id = rc.get("opsradar:siem:last_id")
        except Exception:
            rc = None
            last_id = None
        ...
        if rc is not None:
            try:
                rc.set("opsradar:siem:last_id", str(ship[0].id))
            except Exception:
                pass
...
def backup_database():
    """Nightly pg_dump → /data/uploads/_backups/opsradar-YYYYMMDD.sql.
    ...
    target = os.path.join(backup_dir, f"opsradar-{stamp}.sql.gz")
    ...
    cmd = [
        ...
        "-U", url.username or "opsradar",
        "-d", (url.path or "/opsradar").lstrip("/"),
        "--no-owner",
        "--no-acl",
    ]
...
def check_conformance_drift():
    ...
    try:
        ...
        for event_log in logs:
            ...
            checked += 1
            hist_key = f"opsradar:conformance_history:{event_log.id}"
            try:
