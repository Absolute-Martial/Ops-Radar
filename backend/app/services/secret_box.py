...
Key derivation order:
  ...
  1. ``OPS_RADAR_ENCRYPTION_KEY`` env var — either a raw
     urlsafe-base64 Fernet key or an arbitrary passphrase
     (SHA-256-hashed up to 32 B).
  2. Backward-compatible fallback to ``FLOWMINER_ENCRYPTION_KEY``.
  3. Fall back to ``settings.SECRET_KEY`` — the JWT signing key.
     This is always present (``validate_production_secrets`` rejects
     empty / too-short values), so connector credentials are encrypted
     by default on every install. Operators who want a separate key can
     still set ``OPS_RADAR_ENCRYPTION_KEY`` explicitly.

Rotating one key does not affect the other, but rotating
``SECRET_KEY`` without an explicit encryption key will make existing
encrypted connector configs unreadable — set
``OPS_RADAR_ENCRYPTION_KEY`` before rotating ``SECRET_KEY`` in that
case.
"""
...
def _derive_key() -> bytes | None:
    raw = os.getenv("OPS_RADAR_ENCRYPTION_KEY", "") or os.getenv("FLOWMINER_ENCRYPTION_KEY", "")
    if raw:
        try:
...
if _FERNET is None:
    logger.warning(
        ...
        "stored in plaintext. Set OPS_RADAR_ENCRYPTION_KEY, "
        "FLOWMINER_ENCRYPTION_KEY, or SECRET_KEY."
    )
