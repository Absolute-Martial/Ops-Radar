...
Supports six providers and an auto-fallback order:
  1. ``anthropic``         — Claude via the official anthropic SDK
  2. ``openai``            — GPT via the openai SDK
  3. ``openai_compatible`` — any OpenAI-compatible endpoint via the
                             openai SDK plus a configurable base URL
  4. ``openrouter``        — OpenRouter aggregator (uses the openai SDK with a
  ...
  5. ``ollama``            — local Ollama HTTP API (great for air-gapped)
  6. ``null``              — returns a templated response instead of calling an LLM
...
Environment variables:
  OPS_RADAR_LLM_PROVIDER   anthropic | openai | openai_compatible |
                           openrouter | ollama | null
                           (default: null)
  ...
  OPENAI_BASE_URL          optional base URL for openai_compatible
                           (and as an override for openai)
...
def _provider() -> str:
    db_val = _from_system_settings("llm.provider")
    if db_val:
        return str(db_val).lower()
    env_val = os.getenv("OPS_RADAR_LLM_PROVIDER", "").strip() or os.getenv("FLOWMINER_LLM_PROVIDER", "").strip()
    if env_val:
...
def _model(provider: str | None = None) -> str:
    ...
    env_key_map = {
        ...
        "openai_compatible": ("OPENAI_MODEL", "gpt-4o-mini"),
...
def is_llm_configured() -> bool:
    ...
    if p == "openai_compatible":
        return bool(_api_key(p) and _base_url(p))
...
def _base_url(provider: str | None = None) -> str:
    provider = (provider or _provider()).lower()
    db_val = _from_system_settings("llm.base_url")
    if db_val:
...
def _openrouter_default_headers() -> dict[str, str]:
    ...
    app = os.getenv("OPENROUTER_APP_NAME", "OpsRadar").strip()
    if site:
...
def _openai_client_kwargs(provider: str) -> dict[str, object]:
    kwargs: dict[str, object] = {"api_key": _api_key(provider)}
    ...
    if provider == "openai_compatible":
        if not base_url:
            raise LLMError("openai_compatible provider requires llm.base_url or OPENAI_BASE_URL")
...
def complete(system: str, user: str, *, temperature: float = 0.2) -> str:
    ...
    try:
        ...
        if p == "openai_compatible":
            return _openai_compatible_complete(system, user, temperature=temperature)
        if p == "openrouter":
...
def _openai_complete(system: str, user: str, temperature: float) -> str:
    ...
    model = _model("openai")
    client = OpenAI(**_openai_client_kwargs("openai"))
...
def _openai_compatible_complete(system: str, user: str, temperature: float) -> str:
    try:
        from openai import OpenAI
    except ImportError as e:
        raise LLMError("openai SDK not installed") from e

    model = _model("openai_compatible")
    client = OpenAI(**_openai_client_kwargs("openai_compatible"))
    resp = client.chat.completions.create(
        model=model,
        temperature=temperature,
        messages=[
...
def _openrouter_complete(system: str, user: str, temperature: float) -> str:
    ...
    client = OpenAI(**_openai_client_kwargs("openrouter"))
    resp = client.chat.completions.create(
        model=model,
        temperature=temperature,
        messages=[
...
def _null_complete(system: str, user: str) -> str:
    ...
    return (
        "⚠️ No LLM provider is configured — set OPS_RADAR_LLM_PROVIDER "
...
async def stream(system: str, user: str, *, temperature: float = 0.2) -> AsyncIterator[str]:
    ...
    try:
        ...
        if p == "openai":
            ...
            return
        if p == "openai_compatible":
            async for chunk in _openai_compatible_stream(system, user, temperature):
                yield chunk
            return
        if p == "openrouter":
...
async def _openai_stream(system: str, user: str, temperature: float) -> AsyncIterator[str]:
    try:
        ...
        return
    model = _model("openai")
    client = AsyncOpenAI(**_openai_client_kwargs("openai"))
...
async def _openai_compatible_stream(system: str, user: str, temperature: float) -> AsyncIterator[str]:
    try:
        from openai import AsyncOpenAI
    except ImportError:
        yield _null_complete(system, user)
        return
    model = _model("openai_compatible")
    client = AsyncOpenAI(**_openai_client_kwargs("openai_compatible"))
    stream = await client.chat.completions.create(
        model=model,
        temperature=temperature,
        stream=True,
...
async def _openrouter_stream(system: str, user: str, temperature: float) -> AsyncIterator[str]:
    ...
    try:
        ...
        return
    model = _model("openrouter")
    client = AsyncOpenAI(**_openai_client_kwargs("openrouter"))
    stream = await client.chat.completions.create(
        model=model,
        temperature=temperature,
        stream=True,
...
async def stream_with_tools(
    ...
    *,
    ...
    """
    p = _provider()
    supported = p in ("openai", "openai_compatible", "openrouter")
