...
async def client(test_engine) -> AsyncClient:
    ...
    Session = async_sessionmaker(test_engine, expire_on_commit=False, class_=AsyncSession)
    original_persist = audit_service._persist_audit_entry

    async def _noop_persist_audit_entry(**_kwargs):
        return None
    ...
    audit_service._persist_audit_entry = _noop_persist_audit_entry
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            yield ac
    finally:
        app.dependency_overrides.pop(get_db, None)
        audit_service._persist_audit_entry = original_persist
