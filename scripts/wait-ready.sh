#!/usr/bin/env bash
# wait-ready.sh — block until the OpsRadar stack is reachable.
#
...
PORT="${OPS_RADAR_FRONTEND_PORT:-3000}"
HOST="${OPS_RADAR_HOST:-localhost}"
...
echo "Waiting for OpsRadar at ${URL} (timeout ${TIMEOUT}s)..."
...
while true; do
    ...
    if [[ "$code" == "200" ]]; then
        echo
        echo "Ready. OpsRadar is up at http://${HOST}:${PORT}"
        exit 0
    fi
