#!/usr/bin/env bash
# init-env.sh — generate a fresh .env for OpsRadar with strong secrets.
#
...
#   * OPS_RADAR_ENCRYPTION_KEY — Fernet key (32 random bytes,
...
sed -i.tmp \
    ...
    -e "s|^OPS_RADAR_ENCRYPTION_KEY=.*|OPS_RADAR_ENCRYPTION_KEY=${fernet_key}|" \
    "$ENV_FILE"
rm -f "${ENV_FILE}.tmp"
