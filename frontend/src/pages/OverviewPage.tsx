...
export default function OverviewPage() {
  ...
  if (loading) return <LoadingSpinner size="lg" text="Loading radar…" fullPage />;
    ...
    return (
      <div>
        <PageHeader
          ...
          description="Cross-workspace view of workflow friction, request volume, and active approvals."
        />
        <div className="mt-6">
          <EmptyState
            icon={FolderKanban}
            title="No workflow traces yet"
            description="Load sample request data or upload a workflow trace to start seeing friction metrics, alerts, and automation opportunities here."
            action={
  ...
  return (
    <div>
      <PageHeader
        title="Workflow Radar"
        icon={LayoutDashboard}
        description="Cross-workspace view of workflow friction, delays, and automation opportunities. Click a tile to drill in."
      />
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <ValueCard
          ...
          description={
            initiatives.total > 0
              ...
              : 'No initiatives yet — start tracking savings from any delay point or policy finding.'
          }
          actionLabel={initiatives.total === 0 ? 'Create an initiative' : 'View automations'}
        ...
        <ValueCard
          ...
          value={
            totals.avg_events_per_case > 0
              ? totals.avg_events_per_case.toFixed(1) + ' steps/request'
              : '—'
          }
          description={`${totals.total_activities.toLocaleString()} unique workflow steps across ${totals.event_logs} ready traces.`}
        ...
        <ValueCard
          ...
          value={
            working_capital
              ? `${working_capital.logs_with_cost} trace${working_capital.logs_with_cost === 1 ? '' : 's'} with cost data`
              : 'Not tracked'
          }
          description={
            working_capital
              ? 'Cost per request rolls up from workflow traces where you mapped a cost column. Click to filter workspaces to the ones with cost data.'
              : 'Map a cost column on any workflow trace and this tile will start tracking per-request cost and savings potential.'
          }
          actionLabel={working_capital ? 'See cost-tracked workspaces' : 'Manage workflow traces'}
          onAction={() =>
      ...
      <div className="mt-6 card p-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-[14px] font-semibold text-fg">Recent workflow traces</h2>
            <p className="mt-0.5 text-[12px] text-fg-muted">
              The {recent_event_logs.length} most recently uploaded traces.
            </p>
          </div>
          <Link
            to="/projects"
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 flex items-center gap-3 text-[11px] text-fg-muted">
                  ...
                  <span className="tabular-nums">
                    {log.total_cases.toLocaleString()} requests · {log.total_events.toLocaleString()} steps
                  </span>
                  {log.created_at && (
