...
export default function TemplatesPage() {
  ...
  const builtinCount = templateList.filter((template) => template.is_builtin).length;
  const customCount = templateList.length - builtinCount;
  const activityCoverage = templateList.reduce(
    (total, template) => total + template.expected_activities.length,
    0,
  );
  ...
  return (
    <div>
      <PageHeader
        ...
        icon={LayoutTemplate}
        description="Shape repeatable OpsRadar workflows for approvals, reviews, and exception handling. Templates capture the expected path, KPIs, and anti-patterns operators should watch."
        actions={
          <>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <TemplateMetric label="Templates" value={templateList.length} icon={LibraryBig} />
        <TemplateMetric label="Starter packs" value={builtinCount} icon={Sparkles} />
        <TemplateMetric label="Custom flows" value={customCount} icon={LayoutTemplate} />
        <TemplateMetric label="Mapped stages" value={activityCoverage} icon={Layers3} />
      </div>
      ...
      {templateList.length === 0 ? (
        <div className="mt-16 flex flex-col items-center">
          ...
          <p className="mt-1 max-w-md text-center text-[12px] text-fg-muted">
            Load starter workflows for common business processes, then tailor them to OpsRadar review stages and approval policies.
          </p>
        ...
        <div className="space-y-5">
          <div>
            ...
            <input
              ...
              placeholder="e.g., Invoice exception approval"
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Describe the review path, approvers, and business intent..."
              rows={3}
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., Intake, Validate, Review, Approve, Complete"
              className="input w-full"
            />
            <p className="mt-1.5 text-[11px] text-fg-faint">
              Enter stage names separated by commas
            </p>
          </div>
        </div>
      </Modal>
