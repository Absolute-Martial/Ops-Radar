...
import { Shield, Download, RefreshCw, Search, Activity, Users, Shapes, History } from 'lucide-react';
...
export default function AuditLogPage() {
  ...
  const topResource = summary?.by_resource[0];
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <PageHeader
          title="Audit Log"
          icon={Shield}
          description="Review the immutable OpsRadar activity trail for changes to approvals, templates, intake sources, and workspace administration."
        />
      ...
      {summary && (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Total events" value={summary.total} icon={History} />
          <StatCard label="Action types" value={summary.by_action.length} icon={Activity} />
          <StatCard label="Resource types" value={summary.by_resource.length} icon={Shapes} />
          <StatCard label="Active users" value={summary.top_users.length} icon={Users} />
        </div>
      )}
      ...
      {summary && (
        <div className="grid gap-3 lg:grid-cols-3">
          <InsightCard
            ...
            value={topAction ? `${topAction.action} (${topAction.count})` : 'No events yet'}
            description="Dominant change pattern in the current audit snapshot."
          />
          <InsightCard
            title="Most affected resource"
            value={topResource ? `${topResource.resource_type} (${topResource.count})` : 'No resources yet'}
            description="Where operators and admins are changing the system most."
          />
          <InsightCard
            title="Most active operator"
            value={topUser ? `${topUser.user_email} (${topUser.count})` : 'No identified user'}
            description="Highest-volume actor across logged write operations."
          />
        </div>
      )}
      ...
      <div className="rounded-lg border border-line bg-surface-1 p-3">
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            <label className="mb-1 block text-[11px] font-medium text-fg-muted">Actor email</label>
...
function InsightCard({
  ...
  return (
    <div className="rounded-lg border border-line bg-surface-1 p-4">
      <p className="text-[11px] font-semibold uppercase tracking-wider text-fg-faint">{title}</p>
      <p className="mt-2 text-[13px] font-semibold text-fg break-words">{value}</p>
