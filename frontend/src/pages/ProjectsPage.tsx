...
const FILTERS: FilterDef[] = [
  {
    ...
    icon: Layers,
    description: 'Every workspace you can see',
    matches: () => true,
  },
  {
    key: 'has_logs',
    label: 'With workflow traces',
    icon: FileText,
    description: 'At least one uploaded workflow trace',
  ...
  },
  {
    ...
    label: 'OCEL',
    icon: Boxes,
    description: 'At least one object-centric workflow trace',
  ...
  },
  {
    ...
    label: 'Empty',
    icon: FolderKanban,
    description: 'Workspaces with no workflow traces yet',
    matches: (p) => (p.event_log_count ?? 0) === 0,
  },
];
...
export default function ProjectsPage() {
  ...
  const [showOnboarding, setShowOnboarding] = useState(() => {
    return !(
      localStorage.getItem('opsradar-onboarding-dismissed') ||
      localStorage.getItem('flowfix-onboarding-dismissed')
    );
  });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      addNotification({ type: 'error', title: 'Failed to create workspace' });
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleDelete = async (projectId: string, projectName: string) => {
    ...
    try {
      ...
      addNotification({ type: 'error', title: 'Failed to delete workspace' });
    }
    setOpenMenuId(null);
  };

  if (loading && projects.length === 0) {
    return <LoadingSpinner size="lg" text="Loading workspaces…" fullPage />;
  }
    ...
    <div>
        ...
        <div className="mb-6">
          <OnboardingWizard
            onDismiss={() => {
              ...
              localStorage.setItem('opsradar-onboarding-dismissed', '1');
              localStorage.removeItem('flowfix-onboarding-dismissed');
            }}
      ...
      <div className="mb-6">
        <PageHeader
          title="Workspaces"
          icon={FolderKanban}
          description="Manage the OpsRadar workspaces that hold workflow traces, request datasets, and automation analysis."
          actions={
            demoMode ? null : (
              <>
                <button
      ...
      {projects.length === 0 ? (
        <div className="rounded-xl border border-dashed border-line p-12 text-center">
          ...
          <p className="mt-3 text-[13px] font-semibold text-fg">
            {demoMode ? 'No demo workspaces loaded' : 'No workspaces yet'}
          </p>
          <p className="mx-auto mt-1.5 max-w-md text-[12px] text-fg-muted">
            {demoMode
              ...
              : 'Create your first workspace or explore with sample data to get started.'}
          </p>
          {!demoMode && (
            ...
            <div className="rounded-xl border border-dashed border-line p-8 text-center">
              ...
              <p className="mt-1 text-[12px] text-fg-muted">
                {activeFilter !== 'all' && searchQuery
                  ? `No workspaces in "${FILTERS.find((f) => f.key === activeFilter)?.label}" match "${searchQuery}".`
                  : activeFilter !== 'all'
                    ? `No workspaces match "${FILTERS.find((f) => f.key === activeFilter)?.label}".`
