...
import type { LLMConfigResponse, SystemHealthResponse } from '@/api/client';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (activeTab !== 'health' || !canViewHealth) return;
    let cancelled = false;
  ...
  }, [activeTab, canViewHealth, healthReloadKey]);
  ...
  const handleSaveLLMConfig = async () => {
    ...
    try {
      ...
      if (llmConfig && llmBaseUrl !== (llmConfig.base_url ?? '')) {
        body.base_url = llmBaseUrl.trim();
      }
      if (llmApiKey.trim()) {
  ...
  const adminTabs = [
    { id: 'ai' as const, label: 'AI Workspace', icon: Sparkles },
    ...
    return (
      <div className="mx-auto max-w-2xl">
        <PageHeader
          title="Settings"
          icon={Settings}
          description="OpsRadar workspace controls are read-only in the demo"
        />
        <div className="mt-6 rounded-xl border border-dashed border-line bg-surface-2 p-10 text-center">
          ...
          <p className="mx-auto mt-1.5 max-w-md text-[12px] text-fg-muted">
            ...
            self-host OpsRadar and make yourself an admin on boot.
          </p>
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            ...
            <a
              href="https://demo.opsradar.local"
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        title="Settings"
        icon={Settings}
        description="Manage your OpsRadar identity, workspace controls, AI configuration, and operator diagnostics."
      />
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <SettingsSummaryCard
          ...
          hint="Current workspace access"
          icon={Shield}
        />
        <SettingsSummaryCard
          label="Profile state"
          value={user?.is_active ? 'Active' : 'Inactive'}
          hint="Account availability"
          icon={User}
        />
        <SettingsSummaryCard
          label="AI controls"
          value={canEditLLM ? 'Admin' : 'Restricted'}
          hint="Provider management access"
          icon={Sparkles}
        />
        <SettingsSummaryCard
          label="Health checks"
          value={canViewHealth ? 'Visible' : 'Hidden'}
          hint="Operational diagnostics"
          icon={Activity}
        />
      </div>
        ...
        <div className="mt-6">
          <div className="card p-6">
              ...
              <p className="mt-1 text-[12px] text-fg-muted">
                Update your password to keep your OpsRadar operator account secure.
              </p>
            ...
            <div className="card mt-6 p-6">
              ...
              >
                ...
                <p className="text-[11px] text-fg-muted">
                      Manage operators, roles, and workspace access
                    </p>
                  </div>
                </div>
          ...
          <div className="card p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-[12px] text-fg-muted">
                  Configure the large-language-model provider behind OpsRadar assistance, summaries, and operator guidance. Keys are encrypted at rest and never returned by the API.
                </p>
              </div>
              {llmConfig && (
                <div
                  ...
                  <p className="mt-1 text-[11px] text-fg-faint">
                    ...
                    {llmConfig.provider_source === 'db'
                      ...
                      : llmConfig.provider_source === 'env'
                        ? 'inherited from OPS_RADAR_LLM_PROVIDER env var'
                        : 'unset'}
                  </p>
                </div>
                      ...
                      : llmProvider === 'anthropic'
                          ...
                          : llmProvider === 'openai'
                            ...
                            : llmProvider === 'openai_compatible'
                              ? 'e.g. gpt-4o-mini or your provider model id'
                            : 'e.g. llama3.1'
                    }
                ...
                {(llmProvider === 'openai' || llmProvider === 'openai_compatible') && (
                  <div>
                    ...
                    <input
                      ...
                      placeholder={
                        llmProvider === 'openai'
                          ? 'Optional override, e.g. Azure/OpenAI gateway base URL'
                          : 'Required, e.g. https://your-provider.example.com/v1'
                      }
                      className="input mt-1.5 font-mono"
                    />
                    <p className="mt-1 text-[11px] text-fg-faint">
                      Source: <span className="font-mono">{llmConfig.base_url_source}</span>
                    </p>
                  </div>
                )}
                ...
                {llmProvider !== 'null' && llmProvider !== 'ollama' && (
                  <div>
                    ...
                    <p className="mt-1 flex items-center gap-1 text-[11px] text-fg-faint">
                      ...
                      ({' '}
                      <span className="font-mono">OPS_RADAR_ENCRYPTION_KEY</span>
                      {' '}when set, otherwise a SECRET_KEY-derived fallback). Source:{' '}
            ...
            <div className="card p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-[12px] text-fg-muted">
                    Live status of the components this OpsRadar deployment depends on. Use this to diagnose configuration issues without tailing container logs.
                  </p>
                </div>
                <button
                  type="button"
                      ...
                      ((key === 'smtp' && status.detail.startsWith('disabled')) ||
                        ...
                        (key === 'encryption' &&
                          (status.detail.startsWith('derived from SECRET_KEY') ||
                            status.detail.startsWith('legacy key configured'))));
                    return (
                      <li
                        key={key}
    ...
    <div className="rounded-xl border border-line bg-surface-1 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-[18px] font-semibold text-fg">{value}</p>
